...
fs.copyFileSync('src/App_temp.tsx', file); // start clean
const lines = fs.readFileSync(file, 'utf8').split('\\n');

// The original file is separated properly by newlines.
// Keep lines 0 to 461 (up to the `            ) : (` part)
const before = lines.slice(0, 462).join('\\n');
// Keep lines 904 onwards (from `        ) : activeTab === 'jogadores' ? (` part)
const after = lines.slice(904).join('\\n');

fs.writeFileSync(file, before + '\\n' + newLayout.replace(/\\\\\\`/g, '\`') + '\\n' + after);
console.log('Successfully wrote exact line replacement to App.tsx');
