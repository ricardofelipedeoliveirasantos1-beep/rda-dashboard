...
const getCard = (i) => cardsText.split('[C' + i + ']')[1].split('[/C' + i + ']')[0].trim();
...
for (let i = 1; i <= 9; i++) {
  ...
  while (close > open) {
    if (c.endsWith('</div>')) {
      c = c.substring(0, c.lastIndexOf('</div>')).trim();
      close--;
    } else {
      // Just assume we need to remove the last one wherever it is
      c = c.replace(/<\/div>([^<]*)$/, '$1');
      close--;
    }
  }
  ...
  if (i === 3 || i === 6) {
    c = c.replace('<div className="dashboard-card">', '<div className="dashboard-card" style={{ display: "flex", flexDirection: "column" }}>');
  }
